...
// The slide itself never records a punch directly. It only opens the
// camera in the matching mode and immediately snaps the checkbox back
// to its true (still-unchanged) state — the ACTUAL flip happens only
// once confirmPhoto() in camera.js has a confirmed photo in hand. This
// is what makes an accidental slide harmless: without a real photo
// capture + confirm, nothing about attendance changes at all.

function handlePunchToggleChange(e) {
    const toggle = e.target;
    const wantsIn = toggle.checked; // where the user just slid it to

    // Revert immediately — updateAttendanceStatusUI() (via
    // finishPunchCapture/cancelPunchCapture in camera.js) is the only
    // thing allowed to move it for real, once a punch is confirmed.
    toggle.checked = isPunchedIn();

    if (wantsIn && !isPunchedIn()) {
        openCamera('punch-in');
    } else if (!wantsIn && isPunchedIn()) {
        openCamera('punch-out');
    }
    // Any other combination (e.g. a stray event firing with no real
    // state change) is a no-op — nothing to do.
}
...
function updateAttendanceStatusUI() {
    ...
    if (label) label.textContent = punchedIn ? 'Slide to Punch Out' : 'Slide to Punch In';
